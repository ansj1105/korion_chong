...
- [x] 공통 Data Notes: `users`는 기존 인증 주체, `distributor_signup_applications`는 승인 전 가입신청, `ReferralCode`, `WalletLink`, `ApprovalWorkflow`는 총판 도메인 보조 모델로 사용한다.
...
- [x] 공개 파트너/가맹점 가입신청은 `distributor_signup_applications`에 저장한다.
- [x] `distributor_signup_applications`는 승인 전 신청 레코드이며, 생성 시점에 `users`, `partners`, `merchants` 권한을 생성하거나 활성화하지 않는다.
- [x] 가입신청의 비밀번호는 원문 저장을 금지하고 `distributor_signup_applications.password_hash`에 해시로만 저장한다.
- [ ] 최종 승인 후 본사/상위 검토 API가 필요한 `users`, `partners`, `merchants`, `distributor_contracts`, `distributor_wallet_addresses`를 생성 또는 연결한다.
- [x] `partner_applications`는 기존 마케팅/제휴 신청 호환 테이블로 유지하고, 총판 포털 회원가입의 1차 저장소로 사용하지 않는다.
...
- [x] 승인 전 `대기`, `매장대기`, `결제비활성` 같은 화면 상태는 `distributor_signup_applications.status`와 승인 후 `partners/merchants` 상태를 조합해 표시한다.
...
- [x] Wallet 인증 성공/실패, nonce 발급, 재시도는 ActivityLog 또는 AuditLog에 기록한다.
...
- [x] 공개 파트너/가맹점 가입신청 저장소는 별도 `distributor_signup_applications`를 사용한다.
- [x] 가입신청 생성 시 `users`, `partners`, `merchants` 권한은 생성하지 않고 승인 전 비활성 상태를 유지한다.
- [x] 가입신청 비밀번호는 평문 저장을 금지하고 `distributor_signup_applications.password_hash`에 해시로만 저장한다.
...
- [x] Wallet 인증은 KORION WALLET TRX 주소 기반 nonce 인증을 전제로 하며, 개인키/시드문구/PIN은 입력받지 않는다.
...
- [x] Flyway V116은 `distributor_signup_applications` 생성, V117은 이메일/Wallet 검증 이력 테이블 생성 기준으로 정리한다.

### 14.2 검증 결과

- [x] `korion_chong`: `./gradlew clean test --no-daemon` 통과.
...
- [ ] `distributor_signup_applications.APPROVED` 이후 `users`, `partners`, `merchants`, `distributor_contracts`, `distributor_wallet_addresses`를 생성 또는 연결하는 승인 API를 구현한다.
