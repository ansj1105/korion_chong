...
- [ ] `kori_partners` 프로젝트를 참고 프로젝트로 사용한다.
- [ ] workspace에 없으면 `https://github.com/ansj1105/kori_partners.git`에서 클론한다.
- [ ] 기존 프로젝트의 라우팅, i18n, 컴포넌트 구조, mock JSON hook 패턴을 먼저 파악한다.
- [ ] 기존 메뉴/IA를 임의 재구성하지 않고, 요청된 국가 리더 대시보드 범위 안에서만 변경한다.
- [ ] `kori_partners`에서 이미 진행된 미커밋 변경은 별도 리뷰 전까지 제품 코드로 확정하지 않는다.

### 12.2.1 로그인/회원가입 진입 요구사항

- [ ] Purpose: 권한별 로그인과 파트너/가맹점 회원가입 진입을 제공한다.
- [ ] Permission: 전체 사용자가 접근 가능하다.
- [ ] Main Features: 리더/파트너/가맹점 로그인 카드, 파트너/가맹점 가입 선택, 보안 안내.
- [ ] Input Fields: 없음 또는 진입 선택값.
- [ ] Actions: 로그인 화면 이동, 회원가입 화면 이동.
- [ ] Status: 권한별 라우팅 필요.
- [ ] Validation: 선택한 로그인 카드와 실제 인증 후 역할이 일치해야 한다.
- [ ] Validation: 리더 회원가입 진입은 일반 사용자에게 노출하지 않고 본사 또는 운영 생성 플로우로 제한한다.
- [ ] Validation: 파트너/가맹점 회원가입 진입은 승인 요청 플로우로 연결하고 즉시 권한을 부여하지 않는다.
- [ ] Data Notes: Auth routing, role selection, ActivityLog 시작점으로 사용한다.
...
- [ ] Validation: 본사에서 발급 또는 승인된 국가 리더 계정만 `/leader/dashboard`로 진입할 수 있다.
- [ ] Validation: 계정 정지, 비활성, 승인 전 상태는 로그인 성공 처리하지 않는다.
- [ ] Validation: 2FA가 활성화된 리더 계정은 2FA 코드 검증 전 세션을 발급하지 않는다.
- [ ] Validation: 로그인 유지 옵션은 보안 정책의 만료 시간과 기기 신뢰 정책을 따른다.
- [ ] Data Notes: 로그인 성공 시 `/leader/dashboard`로 이동하고 자기 국가 scope를 적용한다.
...
- [ ] Actions: 로그인, 비밀번호 찾기.
- [ ] Status: `활성`, `비활성`, `정지`.
- [ ] Validation: 활성 상태의 세일즈 파트너만 `/partner/dashboard`로 진입할 수 있다.
- [ ] Validation: 비활성 또는 정지 상태의 파트너는 하위 가맹점과 정산 화면 접근을 차단한다.
- [ ] Validation: 2FA가 활성화된 파트너 계정은 2FA 코드 검증 전 세션을 발급하지 않는다.
- [ ] Validation: 로그인 후 모든 파트너 API는 own merchants scope를 서버에서 적용한다.
- [ ] Data Notes: 로그인 성공 시 `/partner/dashboard`로 이동하고 own merchants scope를 적용한다.
...
- [ ] Validation: 활성 매장에 연결된 가맹점 운영자만 `/merchant/dashboard`로 진입할 수 있다.
...
- [ ] Data Notes: 로그인 성공 시 `/merchant/dashboard`로 이동하고 own store scope를 적용한다.
...
- [ ] Status: `승인요청`, `대기`, `비활성`.
- [ ] Validation: 이메일 인증은 가입신청 제출 전 필수이다.
- [ ] Validation: 아이디와 이메일은 중복 확인을 통과해야 한다.
- [ ] Validation: 리더코드가 있으면 유효한 국가 리더 또는 상위 조직 코드인지 검증한다.
- [ ] Validation: 리더코드가 없으면 본사 직접 계약 신청으로 분류한다.
- [ ] Validation: KORION WALLET TRX 주소는 형식, 중복, nonce 서명 인증 정책을 따른다.
- [ ] Data Notes: 최종 승인과 권한 활성화는 본사 또는 상위 검토 후 처리한다.
- [ ] Data Notes: 가입신청은 즉시 파트너 권한을 부여하지 않고 파트너 신청/검토 레코드로 저장한다.
...
- [ ] Validation: 리더 코드, 파트너 코드, 본사 직접 계약 중 하나의 계약 경로가 필요하다.
- [ ] Validation: 리더/파트너 코드는 유효한 상위 조직과 계약 가능 상태인지 검증한다.
- [ ] Validation: 매장명, 주소, 업종, 증빙자료는 중복 가맹점 검토 대상이다.
- [ ] Validation: KORION WALLET TRX 주소는 형식, 중복, nonce 서명 인증 정책을 따른다.
- [ ] Data Notes: 승인 전 결제 기능은 비활성 상태로 유지한다.
- [ ] Data Notes: 중복 가맹점 의심은 RiskFlag 또는 검토 사유로 기록한다.
- [ ] Data Notes: 가입신청은 즉시 가맹점 권한과 결제 권한을 부여하지 않고 가맹점 신청/검토 레코드로 저장한다.
- [ ] Data Notes: 코드확인, 중복확인, 가입신청 제출, 증빙자료 업로드는 ActivityLog 또는 AuthActivityLog에 기록한다.

### 12.2.7 확인 모달/토스트 요구사항

- [ ] Purpose: 가입, 인증, 중복확인, 제출 결과를 사용자에게 피드백한다.
- [ ] Permission: 전체 사용자가 사용할 수 있다.
- [ ] Main Features: 확인 모달, 오류 모달, 성공 토스트.
- [ ] Input Fields: 해당 없음.
- [ ] Actions: 확인, 취소, 재시도.
- [ ] Status: `성공`, `실패`, `중복`, `인증완료`.
- [ ] Validation: 중복확인, 이메일 인증, Wallet 인증, 가입신청 제출 등 중요 액션은 결과 상태를 명확히 표시한다.
...
- [ ] 공통 Input Fields: 아이디, 비밀번호, 이메일, 인증번호, 휴대폰/WhatsApp, Telegram, 코드, KORION WALLET 주소.
- [ ] 공통 Actions: 로그인, 회원가입 시작, 중복 확인, 인증번호 발송, 인증 확인, Wallet 연동, 가입 신청.
- [ ] 공통 Status: `승인요청`, `검토중`, `자료요청`, `보류`, `승인`, `거절`, `대기`, `비활성`, `Wallet 미연동`, `Wallet 연동 완료`.
- [ ] 공통 Validation: 필수값, 이메일 인증, 아이디/이메일/Telegram/WhatsApp/Wallet 중복 확인, 코드 유효성.
- [ ] 공통 Modal / Toast: 가입 확인, 중복 경고, 인증 완료 토스트, 가입 신청 완료 토스트.
- [ ] 공통 Data Notes: `users`는 기존 인증 주체, `distributor_signup_applications`는 승인 전 가입신청, `ReferralCode`, `WalletLink`, `ApprovalWorkflow`는 총판 도메인 보조 모델로 사용한다.
- [ ] 공통 Dev Notes: 승인 전 권한은 비활성 상태로 유지한다.
- [ ] 공통 Dev Notes: 개인키, 시드문구, PIN 입력은 금지한다.
- [ ] 공통 Dev Notes: role 기반 라우팅은 필수이다.
- [ ] 화면 1. 로그인/회원가입 메인: 가입 신청자와 전체 사용자를 대상으로 권한별 로그인/회원가입 진입 목적과 기능을 정의한다.
- [ ] 화면 2. 리더 로그인: 국가 리더가 본사 발급/승인 계정으로 로그인하고 성공 시 `/leader/dashboard`로 이동한다.
- [ ] 화면 3. 파트너 로그인: 세일즈 파트너가 로그인하고 성공 시 `/partner/dashboard`와 own merchants scope를 적용받는다.
- [ ] 화면 4. 가맹점 로그인: 가맹점 운영자가 로그인하고 성공 시 `/merchant/dashboard`와 own store scope를 적용받는다.
- [ ] 화면 5. 파트너 회원가입: 가입 신청자가 리더 코드 또는 본사 직접 계약 기준으로 파트너 가입을 신청한다.
- [ ] 화면 6. 가맹점 회원가입: 가입 신청자가 리더 코드, 파트너 코드, 본사 직접 계약 중 하나로 가맹점 가입을 신청한다.
- [ ] 화면 7. KORION Wallet 연동: 가입 신청자와 전체 사용자가 KORION WALLET 주소를 연동하고 Wallet 미연동/연동 완료 상태를 확인한다.
- [ ] 화면 8. 이메일 인증/중복 확인: 가입 신청자와 전체 사용자가 이메일 인증과 아이디/이메일/Telegram/WhatsApp/Wallet 중복 확인을 수행한다.
- [ ] 화면 9. 확인 모달/토스트: 가입, 인증, 중복 확인, Wallet 연동, 제출 결과를 공통 모달/토스트 정책으로 피드백한다.
- [ ] API 후보: `POST /api/auth/login`
- [ ] API 후보: `POST /api/auth/signup-applications`
- [ ] API 후보: `POST /api/auth/email-verifications/send`
- [ ] API 후보: `POST /api/auth/email-verifications/confirm`
- [ ] API 후보: `GET /api/auth/availability`
- [ ] API 후보: `POST /api/auth/wallet-links/verify`
- [ ] API 후보: `GET /api/auth/referral-codes/{code}/validate`

### 12.2.9 총판 가입신청 저장소 및 상태 전이 확정안

- [ ] `users` 테이블은 기존 일반 사용자/지갑 계정의 인증 주체로 유지하고, 총판 회원가입/조직/정산 도메인 상태를 직접 소유하지 않는다.
- [ ] 공개 파트너/가맹점 가입신청은 `distributor_signup_applications`에 저장한다.
- [ ] `distributor_signup_applications`는 승인 전 신청 레코드이며, 생성 시점에 `users`, `partners`, `merchants` 권한을 생성하거나 활성화하지 않는다.
- [ ] 가입신청의 비밀번호는 원문 저장을 금지하고 `distributor_signup_applications.password_hash`에 해시로만 저장한다.
- [ ] 최종 승인 후 본사/상위 검토 API가 필요한 `users`, `partners`, `merchants`, `distributor_contracts`, `distributor_wallet_addresses`를 생성 또는 연결한다.
- [ ] `partner_applications`는 기존 마케팅/제휴 신청 호환 테이블로 유지하고, 총판 포털 회원가입의 1차 저장소로 사용하지 않는다.
- [ ] 표준 상태 코드는 프론트/백엔드/API에서 동일하게 사용한다.
...
- [ ] 승인 후 권한 활성화는 신청 상태가 아니라 승인 API의 후속 생성/연결 결과로 처리한다.
- [ ] 화면 라벨 매핑: `REQUESTED=승인요청`, `REVIEWING=검토중`, `NEED_MORE_INFO=자료요청`, `HOLD=보류`, `APPROVED=승인`, `REJECTED=거절`, `CANCELLED=취소`.
- [ ] 승인 전 `대기`, `매장대기`, `결제비활성` 같은 화면 상태는 `distributor_signup_applications.status`와 승인 후 `partners/merchants` 상태를 조합해 표시한다.
- [ ] 최종 승인과 권한 활성화는 분리한다. 신청 `APPROVED`만으로 로그인 권한, 파트너 권한, 가맹점 결제 권한을 발급하지 않는다.
- [ ] 상태 전이는 `ApprovalWorkflow` 또는 상태 이력 테이블에 actor, reason, requested materials, createdAt을 남긴다.

### 12.2.10 WalletLink nonce 인증 확정안

- [ ] Wallet 연동은 KORION Wallet address 기반 인증으로 확정한다.
- [ ] 인증 대상 주소는 KORION WALLET TRX 네트워크 주소이다.
...
- [ ] 서버는 서명 원문, 개인키, 시드문구, PIN을 저장하거나 요구하지 않는다.
- [ ] Wallet 주소 중복 확인은 승인 완료 계정뿐 아니라 `REQUESTED`, `REVIEWING`, `NEED_MORE_INFO`, `HOLD`, `APPROVED` 상태의 신청 건까지 포함한다.
- [ ] Wallet 인증 성공/실패, nonce 발급, 재시도는 ActivityLog 또는 AuditLog에 기록한다.

### 12.3 국가 리더 API/대시보드 작업큐

- [ ] Purpose: 국가 리더의 담당 국가 운영 현황을 요약한다.
- [ ] Permission: 국가 리더만 접근 가능하다.
- [ ] Validation: 국가 리더는 본인에게 배정된 국가 scope 데이터만 조회할 수 있다.
- [ ] Input Fields: 기간, 국가 scope.
- [ ] Main Features: KPI, 하부 조직, 월 거래량, 수수료, 리스크 알림.
- [ ] Actions: 상세 이동, 리포트 이동.
- [ ] Data Notes: `LeaderProfile`, `Partner summary`, `Merchant summary`를 기본 데이터 단위로 사용한다.
- [ ] API는 국가 scope 권한 검증 후 응답해야 하며, 프론트 필터만 믿지 않는다.
- [ ] 대시보드 데이터는 정산/거래 원천 데이터를 직접 계산하지 않고, API가 제공하는 summary 응답을 표시한다.
...
- [ ] Auth context: `leaderId`, 허용 `countryScopes`.
- [ ] Response: `leaderProfile`, `kpis`, `organizationSummary`, `monthlyVolume`, `feeSummary`, `riskAlerts`.
- [ ] Error: 요청한 `countryScope`가 리더 권한 범위 밖이면 `403` 또는 표준 권한 오류를 반환한다.
...
- [ ] Purpose: 승인된 하부 파트너 목록과 성과를 관리한다.
- [ ] Permission: 국가 리더만 접근 가능하다.
- [ ] Main Features: 파트너 목록, 파트너 상세, 활동 메모, 정지 요청, 권한 변경 요청.
- [ ] Input Fields: 검색, 상태, 지역, 메모.
- [ ] Actions: 상세보기, 본사 정지 요청.
- [ ] Status: `활성`, `비활성`, `정지요청`.
- [ ] Validation: 국가 리더는 본인 국가 scope와 하위 조직에 속한 승인 파트너만 조회할 수 있다.
- [ ] Validation: 정식 승인되지 않은 파트너 후보는 파트너 관리 목록에 포함하지 않는다.
...
- [ ] Query: `keyword`, `status`, `region`, `countryScope`, `page`, `size`.
- [ ] Response: 승인된 하부 파트너 목록, 성과 요약, 상태, 최근 활동일.
...
- [ ] Action: 임시저장.
- [ ] `POST /api/leader/settlement-requests/{requestId}/submit`
- [ ] Action: 본사 정산 요청.
...
- [ ] Error: 정산 가능금액 초과 신청이면 표준 검증 오류를 반환한다.
- [ ] OpenAPI 문서와 contract test를 같이 작성한다.

### 12.15 정산내역 작업큐

- [ ] Purpose: 정산 요청, 승인, 지급, 보류/조정 내역을 확인한다.
- [ ] Permission: 국가 리더만 접근 가능하다.
- [ ] Main Features: 정산 테이블, 상태 필터, 상세 패널.
- [ ] Input Fields: 검색, 기간, 상태.
- [ ] Actions: 상세보기, 다운로드.
- [ ] Status: `지급완료`, `보류`, `조정필요`.
- [ ] Validation: 국가 리더는 본인 국가 scope와 하위 조직 관련 정산내역만 조회할 수 있다.
- [ ] Validation: 다운로드도 조회 권한과 동일한 국가 scope/조직 필터를 적용해야 한다.
- [ ] Data Notes: `SettlementRequest`와 `Commission`을 연결해 요청금액, 승인금액, 지급금액, 보류/조정 금액을 표시한다.
- [ ] Data Notes: 보류/조정 내역은 원 거래, 보류 사유, 조정 사유, 처리 상태를 함께 표시한다.
- [ ] Data Notes: 지급완료 내역은 KORION WALLET 지급 주소와 온체인 거래 식별자 또는 외부 지급 참조값을 연결한다.
- [ ] Audit: 상세 조회와 다운로드 이력을 남긴다.

### 12.16 정산내역 API 후보

- [ ] `GET /api/leader/settlement-history`
- [ ] Query: `keyword`, `periodFrom`, `periodTo`, `status`, `countryScope`, `page`, `size`.
- [ ] Response: 정산내역 테이블, `SettlementRequest` 요약, `Commission` 요약, 지급 상태, 보류/조정 상태.
- [ ] `GET /api/leader/settlement-history/{settlementRequestId}`
- [ ] Response: 정산 요청 상세, 승인/보류/조정 이력, 연결된 Commission 목록, 지급 내역.
...
- [ ] Purpose: 하위 파트너와 가맹점에게 공지를 발송한다.
- [ ] Permission: 국가 리더만 접근 가능하다.
- [ ] Main Features: 대상 선택, 채널 선택, 즉시/예약 발송, 읽음확인.
- [ ] Input Fields: 제목, 유형, 대상, 내용, 채널, 예약시간.
- [ ] Actions: 임시저장, 미리보기, 발송.
- [ ] Status: `예약대기`, `발송중`, `완료`, `실패`.
- [ ] Validation: 타 국가 또는 타 조직 대상에게 발송할 수 없다.
- [ ] Validation: 발송 대상은 리더의 국가 scope와 하위 파트너/가맹점 관계로 서버에서 검증한다.
- [ ] Validation: 예약시간은 현재 시각 이후여야 한다.
- [ ] Validation: 발송 전 제목, 유형, 대상, 내용, 채널 필수값을 검증한다.
- [ ] Data Notes: 리더 공지는 하위 파트너와 가맹점 수신자 단위로 발송/읽음 상태를 저장한다.
- [ ] Data Notes: 채널은 포털 알림, 이메일, Telegram 등 정책 데이터로 관리한다.
- [ ] Data Notes: 발송 실패는 수신자별 실패 사유와 재시도 가능 여부를 저장한다.
- [ ] Audit: 임시저장, 미리보기, 발송, 예약 변경, 발송 실패, 읽음확인 조회 이력을 남긴다.
...
- [ ] Error: 예약시간이 과거이면 표준 검증 오류를 반환한다.
- [ ] OpenAPI 문서와 contract test를 같이 작성한다.
...
- [ ] `POST /api/partner/settlements/requests`
- [ ] Body: 기간, KORION WALLET, 신청금액.
- [ ] Action: 정산신청.
...
- [ ] Action: 읽음 처리.
- [ ] `POST /api/partner/notices/outbox`
- [ ] Body: 제목, 대상, 내용, 채널.
- [ ] Action: 하위 가맹점 공지 발송.
...
- [ ] Error: own merchant scope 밖 수신자가 포함되면 `403` 또는 표준 권한 오류를 반환한다.
- [ ] OpenAPI 문서와 contract test를 같이 작성한다.
...
- [ ] Purpose: 매장 운영자가 본인 매장의 운영/결제 상태를 확인한다.
- [ ] Permission: 가맹점만 접근 가능하다.
- [ ] Main Features: KPI, 거래량, QR/NFC/BLE 상태, 최근 결제.
- [ ] Input Fields: 기간, 결제수단.
- [ ] Actions: 거래 상세보기.
- [ ] Status: `결제완료`, `Sync대기`, `Sync실패`.
- [ ] Validation: 가맹점은 본인 매장 데이터만 조회할 수 있다.
- [ ] Validation: 매장 운영자 계정과 merchant/store scope를 서버에서 검증한다.
- [ ] Data Notes: QR/NFC/BLE 결제수단별 상태와 최근 결제 목록은 동일한 권한 scope를 적용한다.
...
- [ ] Response: KPI, 거래량, QR/NFC/BLE 상태, 최근 결제, Sync 상태 요약.
- [ ] `GET /api/merchant/payments`
- [ ] Query: `periodFrom`, `periodTo`, `paymentMethod`, `status`, `page`, `size`.
- [ ] Response: 본인 매장 결제 목록, 결제수단, 결제상태, Sync 상태, 거래금액, 발생 시각.
...
- [ ] Purpose: 매장 정보, 결제 설정, 상품 상태를 확인하고 수정 요청한다.
- [ ] Permission: 가맹점만 접근 가능하다.
- [ ] Main Features: 매장기본정보, 주소, 업종, KORION WALLET, 상품상태.
- [ ] Input Fields: 매장명, 주소, 연락처, 업종, 상품정보.
- [ ] Actions: 저장, 수정요청, Wallet 확인.
- [ ] Status: `검토중`, `활성`, `비활성`.
- [ ] Validation: 가맹점은 본인 매장 정보만 조회하고 수정 요청할 수 있다.
...
- [ ] Response: 매장기본정보, 주소, 연락처, 업종, KORION WALLET 상태, 상품상태, 검토 상태.
...
- [ ] Purpose: 본사/상위 조직 공지와 계정 활동 로그를 확인한다.
- [ ] Permission: 가맹점만 접근 가능하다.
- [ ] Main Features: 공지수신, 읽음상태, 프로필, 활동로그.
- [ ] Input Fields: 검색, 기간, 유형.
- [ ] Actions: 상세보기, 프로필 저장.
- [ ] Status: `읽음`, `미확인`, `성공`, `실패`.
- [ ] Validation: 가맹점은 본인 매장 또는 본인 계정으로 수신된 공지만 조회할 수 있다.
- [ ] Validation: 계정 활동 로그는 본인 계정과 본인 매장 scope 안에서만 조회할 수 있다.
...
- [ ] `/partner/requests/merchant` 화면에서 가맹점 등록 요청 목록과 상태를 제공한다.
...
- [ ] Auth routing API 또는 로그인 성공 응답은 사용자 role, partnerId, merchantId, country scope를 명확히 반환한다.
- [ ] 로그인 성공 후 role selection과 실제 계정 권한이 불일치하면 권한 오류 또는 재라우팅 응답을 반환한다.
- [ ] 파트너/가맹점 회원가입 진입은 신청 레코드 생성 플로우로 연결하고 즉시 권한을 부여하지 않는다.
- [ ] 리더 계정 생성은 본사 승인/생성 플로우로 제한하고 공개 회원가입 API에서 생성하지 않는다.
- [ ] 로그인 성공/실패, role selection, 역할 불일치, 회원가입 진입 선택은 ActivityLog 또는 AuthActivityLog에 기록한다.
- [ ] 로그인/회원가입 진입에서 민감 payload, 토큰, KORION WALLET 서명 원문은 로그에 저장하지 않는다.
- [ ] 리더 로그인 API는 아이디/비밀번호 검증 후 계정 상태와 국가 리더 승인 상태를 확인한다.
...
- [ ] 리더 로그인 성공 응답에는 leaderId, role, countryScopes, redirectPath=`/leader/dashboard`를 포함한다.
- [ ] 리더 로그인 성공/실패, 2FA 성공/실패, 비밀번호 찾기 요청은 ActivityLog 또는 AuthActivityLog에 기록한다.
- [ ] 파트너 로그인 API는 아이디/비밀번호 검증 후 계정 상태와 세일즈 파트너 승인 상태를 확인한다.
- [ ] 파트너 로그인 API는 2FA 활성 계정에 대해 2FA 검증 완료 전 최종 세션을 발급하지 않는다.
- [ ] 파트너 비활성/정지 상태에서는 하위 가맹점 목록, 정산, 공지 발송 권한을 발급하지 않는다.
- [ ] 파트너 로그인 성공 응답에는 partnerId, role, ownMerchantScope, redirectPath=`/partner/dashboard`를 포함한다.
...
- [ ] 가맹점 로그인 API는 아이디/비밀번호 검증 후 계정 상태, 매장 활성 상태, 결제 가능 상태를 확인한다.
- [ ] 가맹점 로그인 API는 2FA 활성 계정에 대해 2FA 검증 완료 전 최종 세션을 발급하지 않는다.
- [ ] 매장 비활성 또는 결제 제한 상태에서는 결제/정산/매장 운영 권한을 정책에 따라 제한한다.
- [ ] 가맹점 로그인 성공 응답에는 merchantId, storeId, role, ownStoreScope, paymentStatus, redirectPath=`/merchant/dashboard`를 포함한다.
- [ ] 가맹점 로그인 성공/실패, 2FA 성공/실패, 비밀번호 찾기 요청은 ActivityLog 또는 AuthActivityLog에 기록한다.
- [ ] 가맹점 세션으로 호출되는 API는 own store scope를 서버에서 재검증한다.
- [ ] 파트너 회원가입 API는 아이디/이메일 중복확인과 이메일 인증번호 발송/검증을 제공한다.
- [ ] 파트너 가입신청 제출 API는 이메일 인증 완료 여부를 서버에서 검증한다.
- [ ] 리더코드가 있는 가입신청은 유효한 리더 또는 상위 조직 코드와 연결하고, 없으면 본사 직접 계약 신청으로 저장한다.
- [ ] 파트너 가입신청은 `승인요청` 또는 `대기` 상태로 저장하고 로그인 권한/파트너 권한을 즉시 활성화하지 않는다.
- [ ] 최종 승인과 권한 활성화는 본사 또는 상위 검토 API에서만 처리한다.
- [ ] 파트너 가입신청의 KORION WALLET은 TRX 주소 형식, 중복, nonce 서명 인증 정책을 적용한다.
- [ ] 중복확인, 인증발송, 이메일 인증 성공/실패, 가입신청 제출은 ActivityLog 또는 AuthActivityLog에 기록한다.
- [ ] 가맹점 회원가입 API는 리더 코드/파트너 코드 확인과 본사 직접 계약 선택을 검증한다.
- [ ] 가맹점 가입신청 제출 API는 아이디/이메일 중복, 매장명/주소 중복, 증빙자료 필수 여부를 서버에서 검증한다.
- [ ] 가맹점 가입신청은 `승인요청`, `매장대기`, `결제비활성` 상태로 저장하고 승인 전 결제 기능을 활성화하지 않는다.
- [ ] 중복 가맹점 의심은 RiskFlag 또는 검토 사유로 저장하고 본사 또는 상위 조직 검토 대상에 포함한다.
- [ ] 가맹점 가입신청의 KORION WALLET은 TRX 주소 형식, 중복, nonce 서명 인증 정책을 적용한다.
- [ ] 가맹점 최종 승인과 결제 기능 활성화는 본사 또는 상위 검토 API에서만 처리한다.
- [ ] 코드확인, 중복확인, 가입신청 제출, 증빙자료 업로드는 ActivityLog 또는 AuthActivityLog에 기록한다.
- [ ] 가입, 인증, 중복확인, 제출 API는 프론트 공통 모달/토스트가 사용할 수 있는 표준 결과 코드와 사용자 표시용 메시지 키를 반환한다.
- [ ] 중요 액션은 requestId 또는 idempotency key를 받아 중복 제출과 재시도를 추적한다.
- [ ] 성공/실패/중복/인증완료 결과는 ActivityLog 또는 AuditLog에 기록한다.
- [ ] 보안상 민감한 오류 원문은 API 응답과 로그 metadata에서 마스킹한다.
- [ ] 재시도 가능한 오류와 재시도 불가능한 검증 오류를 구분해 응답한다.
- [ ] 회원가입 신청은 `distributor_signup_applications`를 기준으로 저장하고 승인 전 실제 권한을 활성화하지 않는다.
- [ ] `distributor_signup_applications` 상태는 `REQUESTED`, `REVIEWING`, `NEED_MORE_INFO`, `HOLD`, `APPROVED`, `REJECTED`, `CANCELLED` 전이를 기준으로 처리한다.
- [ ] `partner_applications`는 기존 마케팅/제휴 신청 호환 테이블로 유지하고, 총판 포털 회원가입 상태의 기준으로 사용하지 않는다.
- [ ] 승인과 권한 활성화는 분리하고, `distributor_signup_applications.APPROVED`만으로 로그인 권한과 role 권한을 발급하지 않는다.
- [ ] 회원가입 API는 비밀번호 원문을 로그/DB에 저장하지 않고 `password_hash`만 저장한다.
- [ ] role 기반 라우팅은 `users`, `partners`, `merchants`, `distributor_contracts`의 승인/활성 상태를 함께 검증한다.
- [ ] 코드 유효성 검증은 `ReferralCode`와 계약 경로 정책을 기준으로 수행한다.
- [ ] Wallet 연동은 `WalletLink`로 저장하고 개인키/시드문구/PIN은 입력받거나 저장하지 않는다.
- [ ] Wallet 연동은 KORION Wallet address 기반 인증이며, TRX 주소와 nonce 서명 검증 결과를 `WalletLink`에 저장한다.
- [ ] Wallet 주소 중복 검증은 활성 계정뿐 아니라 진행 중인 신청 상태까지 포함한다.
...
- [ ] 정산 신청 제출 시 임시저장 금액을 신뢰하지 않고 서버에서 재계산한다.
- [ ] 본사 검토중/승인/지급완료/보류 상태 전이를 서버에서 관리한다.
...
- [ ] 리더 공지 발송 API는 수신자의 국가 scope와 하위 조직 관계를 서버에서 검증한다.
- [ ] 공지 발송은 notice 본문과 recipient delivery 상태를 분리 저장한다.
- [ ] 예약 발송은 scheduler 또는 outbox worker에서 처리한다.
- [ ] 채널별 발송 실패는 수신자별 실패 사유와 재시도 상태를 저장한다.
- [ ] 읽음확인은 수신자별 read receipt로 저장한다.
- [ ] 타 국가/타 조직 대상이 하나라도 포함되면 발송 요청 전체를 거절한다.
...
- [ ] 파트너 대시보드 API는 partnerId 기준 own merchant scope를 서버에서 검증한다.
...
- [ ] 파트너 정산 신청 상태 전이는 `REQUESTED -> APPROVED/REJECTED -> PAID` 기준으로 관리한다.
- [ ] 파트너 지급내역은 지급 대상 KORION WALLET 주소와 온체인 거래 식별자 또는 외부 지급 참조값을 연결한다.
- [ ] 파트너 정산 신청 제출 시 정산가능수수료를 서버에서 재계산한다.
...
- [ ] `distributor_signup_applications` 승인 후 `users`, `partners`, `merchants`, `distributor_contracts`를 생성 또는 연결하는 승인 API 범위를 확정한다.
...
- [ ] 파트너 가입신청은 `distributor_signup_applications` 저장 기준으로 API/프론트 필드 매핑을 확정한다.
- [ ] 리더코드 형식과 본사 직접 계약 신청의 구분 값을 확정한다.
- [ ] 파트너 가입신청 상태는 `distributor_signup_applications` 표준 상태 전이와 화면 라벨 매핑을 적용한다.
...
- [ ] 가맹점 가입신청은 `distributor_signup_applications` 저장 기준으로 API/프론트 필드 매핑을 확정한다.
...
- [ ] `users`, `partners`, `merchants`, `distributor_signup_applications`, `ReferralCode`, `WalletLink`, `ApprovalWorkflow`의 실제 테이블명과 소유 서비스를 확정한다.
