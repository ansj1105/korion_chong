...
- [ ] 공통 Data Notes: `User`, `AdminAccount`, `SignupApplication`, `ReferralCode`, `WalletLink`, `ApprovalWorkflow`.
...
### 12.2.9 partner_applications 상태 전이 확정안

- [ ] `partner_applications`는 파트너/가맹점 가입신청의 1차 저장소로 사용한다.
- [ ] 표준 상태 코드는 프론트/백엔드/API에서 동일하게 사용한다.
- [ ] 상태 전이: `REQUESTED -> REVIEWING -> NEED_MORE_INFO/HOLD/APPROVED/REJECTED`.
- [ ] 자료요청 보완 후 상태 전이: `NEED_MORE_INFO -> REVIEWING`.
- [ ] 보류 재검토 상태 전이: `HOLD -> REVIEWING/REJECTED`.
- [ ] 승인 후 권한 활성화 상태 전이: `APPROVED -> ACTIVATED`.
- [ ] 비활성 처리 상태 전이: `ACTIVATED -> INACTIVE`.
- [ ] 화면 라벨 매핑: `REQUESTED=승인요청`, `REVIEWING=검토중`, `NEED_MORE_INFO=자료요청`, `HOLD=보류`, `APPROVED=승인`, `REJECTED=거절`, `PENDING=대기`, `INACTIVE=비활성`, `ACTIVATED=활성`.
- [ ] 기존 `partner_applications.status`에 없는 `REQUESTED`, `REVIEWING`, `HOLD`, `ACTIVATED`, `INACTIVE`는 DB 마이그레이션 보강 대상이다.
- [ ] 기존 코드 호환 매핑: `SUBMITTED -> REQUESTED`, `NEED_MORE_INFO -> NEED_MORE_INFO`, `SALES_PARTNER_APPROVED -> APPROVED`, `MERCHANT_APPROVED -> APPROVED`, `REJECTED -> REJECTED`, `SUSPENDED/TERMINATED -> INACTIVE`.
- [ ] 최종 승인과 권한 활성화는 분리한다. `APPROVED`는 심사 승인이고, 실제 로그인/권한 부여는 `ACTIVATED` 이후에만 허용한다.
...
- [ ] 회원가입 신청은 `SignupApplication`을 기준으로 저장하고 승인 전 실제 권한을 활성화하지 않는다.
- [ ] `partner_applications` 상태는 `REQUESTED`, `REVIEWING`, `NEED_MORE_INFO`, `HOLD`, `APPROVED`, `REJECTED`, `ACTIVATED`, `INACTIVE` 전이를 기준으로 처리한다.
- [ ] `partner_applications` 기존 상태 코드는 신규 표준 상태 코드로 마이그레이션 또는 호환 매핑한다.
- [ ] 승인과 권한 활성화는 분리하고, `ACTIVATED` 전에는 로그인 권한과 role 권한을 발급하지 않는다.
- [ ] role 기반 라우팅은 `User`, `AdminAccount`, `SignupApplication` 상태를 함께 검증한다.
...
- [ ] `partner_applications`를 파트너/가맹점 가입신청 저장소로 사용할 때 필요한 상태 코드 DB 마이그레이션 범위를 확정한다.
...
- [ ] `User`, `AdminAccount`, `SignupApplication`, `ReferralCode`, `WalletLink`, `ApprovalWorkflow`의 실제 테이블명과 소유 서비스를 확정한다.
